...
class ApiService {
   ...
   */
  async getInterviewStatus(interviewId: string): Promise<InterviewStatusResponse> {
    const response = await fetch(`${this.baseUrl}/api/interviews/${interviewId}/status`);
  ...
  async getInsights(interviewId: string): Promise<Insights> {
    const response = await fetch(`${this.baseUrl}/api/interviews/${interviewId}/insights`);
    ...
    eventSource.addEventListener('message', (event) => {
      ...
      try {
        ...
        if (data.type === 'message') {
          console.log('💬 Interview message:', data.payload.agent, '-', data.payload.message.substring(0, 50) + '...');
